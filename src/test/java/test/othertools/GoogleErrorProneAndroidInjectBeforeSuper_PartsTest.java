...
public class GoogleErrorProneAndroidInjectBeforeSuper_PartsTest extends TestCase {
    ...
    static $anno $SUPPRESS_WARNING_ANNO = $anno.of("@SuppressWarnings(\"AndroidInjectionBeforeSuper\")");
    ...
    static _source FAILURES = _source.of(
    ...
    public void testFindExs(){
        assertEquals(4, $injectEx.count(FAILURES) );
        assertEquals(2, $onAttachEx.count(FAILURES) );
        assertEquals(2, $onCreateEx.count(FAILURES) );
        ...
        assertEquals(4, $injectAfterSuper.count(FAILURES));
    ...
    public void testFullThingInOneSwoop(){
        ...
        _source SUPPRESS_ANNO = FAILURES.copy();
    ...
    public void testOnCreateOnAttachMethod(){
        ...
        _class _cp = $method.of().forEachIn( _FAIL_InFragment.copy(), m -> m.anno( $SUPPRESS_WARNING_ANNO.draft()));
        assertFalse( $METHODS.isIn(_cp) ); //wont match because @SuppressWarnings anno
        _cp = $method.of().forEachIn( _FAIL_InjBeforeSuperInBetween.copy(), m -> m.anno( $SUPPRESS_WARNING_ANNO.draft()));
        assertFalse( $METHODS.isIn(_cp) ); //wont match because @SuppressWarnings anno
        _cp = $method.of().forEachIn( _FAIL_InjectBeforeSuperOnAttach.copy(), m -> m.anno( $SUPPRESS_WARNING_ANNO.draft()));
        assertFalse( $METHODS.isIn(_cp) ); //wont match because @SuppressWarnings anno
        _cp = $method.of().forEachIn( _FAIL_InjectBeforeSuperActivity.copy(), m -> m.anno( $SUPPRESS_WARNING_ANNO.draft()));
        assertFalse( $METHODS.isIn(_cp) ); //wont match because @SuppressWarnings anno
    }
