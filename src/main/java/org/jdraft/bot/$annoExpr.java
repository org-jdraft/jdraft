...
public class $annoExpr
     ...
     */
    public Select.$botSetSelect<$pair, _annoExpr, _entryPair> pairs =
    ...
    public $annoExpr $pairs(List<$pair> $keyValuePairs) {
        this.pairs.add($keyValuePairs.toArray(new $pair[0]));
        return this;
    }
     ...
     * @return
     */
    public $annoExpr $pair($pair $keyValuePair) {
     ...
     * i.e. @A(key="value")
     */
    public static class $pair extends $baseBot<_entryPair, $pair>
            implements $bot<MemberValuePair, _entryPair, $pair> {
        ...
        public static $pair of(_entryPair _mv) {
            return new $pair(_mv.getName(), _mv.getValue().ast());
        }

        public static $pair of(Predicate<_entryPair> matchFn) {
            $pair $mv = new $pair();
