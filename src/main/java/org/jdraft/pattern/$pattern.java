...
public interface $pattern<P, $P extends $pattern>{
        ...
        for(int i=0;i<$ps.length; i++) {
            ...
            Predicate<P> pp = n -> {
                if (n instanceof Node) {
                    ...
                    return Walk.isParent(((_tree._node) n).node(), c -> $p.match(c));
                } else if (n instanceof _body) {
    ...
    default $P $isParent($pattern... $ps ){
        return $and(n -> {
            if (n instanceof Node) {
                ...
                return Walk.isParent( ((_tree._node)n).node(), c->Arrays.stream($ps).anyMatch($p->$p.match(c)) );
            } else if (n instanceof _body) {
    ...
    default $P $isParent(Predicate<Node> parentMatchFn ){
        return $and(n -> {
            if( n instanceof _java._domain){
                return Walk.isParent( ((_tree._node)n).node(), parentMatchFn);
            }
    ...
    default $P $hasNoAncestor( int levels, Predicate<Node>ancestorMatchFn){
        return $not(n-> {
            if( n instanceof Node ) {
                ...
                return ((_tree._node)n).node().stream($.PARENTS).limit(levels).anyMatch( ancestorMatchFn );
            } else if (n instanceof _body) {
    ...
    default $P $hasNoAncestor( int levels, $pattern...$ps ){
        return $not(n-> {
            if( n instanceof Node ) {
                ...
                return ((_tree._node)n).node().stream($.PARENTS).limit(levels).anyMatch(c-> Arrays.stream($ps).anyMatch($p ->$p.match(c)));
            }else if (n instanceof _body) {
    ...
    default $P $hasNoChild( $pattern...$ps){
        return $not(n-> {
            if( n instanceof Node ){
                ...
                return ((_tree._node)n).node().getChildNodes().stream().anyMatch(c -> Arrays.stream($ps).anyMatch($p->$p.match(c)));
            } else if( n instanceof _body){
    ...
    default $P $hasChild( Class... childClassTypes ){
        return $and(n-> {
            if( n instanceof Node ){
                ...
                return ((_tree._node)n).node().getChildNodes().stream().anyMatch(c -> Walk.isNodeOfType(c, childClassTypes) );
            } else if( n instanceof _body){
    ...
    default $P $hasChild( Predicate<Node> childMatchFn ){
        return $and(n-> {
            if( n instanceof Node ){
                ...
                return ((_tree._node)n).node().getChildNodes().stream().anyMatch(c -> childMatchFn.test(c) );
            } else if( n instanceof _body){
    ...
    default $P $hasDescendant( int depth, Predicate<Node> descendantMatchFn ){
        return $and(n-> {
            if( n instanceof Node ){
                ...
                return Walk.matchDescendant( ((_tree._node)n).node(), depth, descendantMatchFn);
            } else if( n instanceof _body){
    ...
    default $P $hasDescendant( int depth, $pattern... $ps ){
        ...
        return $and(n->{
            if( n instanceof Node ){
                ...
                return Walk.matchDescendant( ((_tree._node)n).node(), depth, c->Arrays.stream($ps).anyMatch($p->$p.match(c)));
            } else if( n instanceof _body){
     ...
     * @return
     */
    default boolean isIn( _project _codeProvider ){
     ...
     * @return
     */
    default boolean isIn(_project _codeProvider, Predicate<P> matchFn){
     ...
     * @return
     */
    default P firstIn( _project _codeProvider ){
     ...
     * @return
     */
    default P firstIn(_project _codeProvider, Predicate<P> matchFn){
        P found = null;
        List<_codeUnit> _lc = _codeProvider.list();
    ...
    default P firstIn(_java._domain _j, Predicate<P> nodeMatchFn){
        if( _j instanceof _codeUnit){
            ...
            return firstIn(_t.node(), nodeMatchFn);
        }
        if( _j instanceof _tree._node) {
            return firstIn(((_tree._node) _j).node());
        }
     ...
     * @return
     */
    default <S extends selected> S selectFirstIn(_project _codeProvider ){
        List<_codeUnit> _cs = _codeProvider.list();
     ...
     * @return a List of P that match the query
     */
    default List<P> listIn( _project _cus ){
        List<P> found = new ArrayList<>();
    ...
    default Stream<P> streamIn( _project _cus ){
     ...
     * @return list of matching P for the query
     */
    default <_J extends _java._domain> List<P> listIn(_project _codeProvider, Predicate<P> nodeMatchFn){
        List<P> found = new ArrayList<>();
     ...
     * @return
     */
    default <_J extends _java._domain> Stream<P> streamIn(_project _codeProvider, Predicate<P> nodeMatchFn){
    ...
    default List<P> listIn(_java._domain _j) {
        if( _j instanceof _codeUnit){
            ...
            _type _t = (_type)_j; //only possible
            return listIn(_t.node()); //return the TypeDeclaration, not the CompilationUnit
        }
        if( _j instanceof _tree._node) {
    ...
    default List<P> listIn(_java._domain _j, Predicate<P>nodeMatchFn){
        if( _j instanceof _codeUnit){
            ...
            _type _t = (_type)_j; //only possible
            return listIn(_t.node(), nodeMatchFn); //return the TypeDeclaration, not the CompilationUnit
        }
        if( _j instanceof _tree._node) {
            return listIn(((_tree._node) _j).node(), nodeMatchFn);
        }
        if( _j instanceof _body ){
     ...
     * @return
     */
    default <S extends selected> List<S> listSelectedIn(_project _codeProvider){
        List<S> sel = new ArrayList<>();
     ...
     * @return
     */
    default List<_codeUnit> forEachIn(_project _cus, Consumer<P> nodeActionFn ){
    ...
    default <_J extends _java._domain> _J forEachIn(_J _j, Predicate<P> nodeMatchFn, Consumer<P> nodeActionFn){
        if( _j instanceof _codeUnit){
            ...
            _type _t = (_type) _j; //only possible
            forEachIn(_t.node(), nodeMatchFn, nodeActionFn); //return the TypeDeclaration, not the CompilationUnit
            return _j;
        }
        if( _j instanceof _tree._node) {
            forEachIn(((_tree._node) _j).node(), nodeMatchFn, nodeActionFn);
            return _j;
        }
        if( _j instanceof _body ){
     ...
     * @return
     */
    default int countIn(_project _codeProvider){
     ...
     * @param _cus the collection to search through
     * @return
     */
    ...
    default void describeIn(Node astNode) {
        forEachIn(astNode, e-> Print.tree((_tree._node)e));
    }

    default void describeIn(Class<?> clazz) {
        forEachIn(Ast.of(clazz), e-> {
            Print.tree( (_tree._node) e); System.out.println();});
    }
     ...
     * @return
     */
    default List<_codeUnit> removeIn(_project _cp ){
     ...
     * @return
     */
    default List<_codeUnit> removeIn(_project _cp, Predicate<P>nodeMatchFn){
    ...
    default <_J extends _java._domain> _J removeIn(_J _j, Predicate<P> nodeMatchFn){
        if( _j instanceof _codeUnit){
            ...
            _type _t = (_type) _j; //only possible
            removeIn(_t.node(), nodeMatchFn); //return the TypeDeclaration, not the CompilationUnit
            return _j;
        }
        removeIn(((_tree._node) _j).node(), nodeMatchFn);
        return _j;
    }
        ...
        return forEachIn(astNode, s-> {
            if( nodeMatchFn.test( s ) ){
                if( s instanceof Node ){
                    ...
                    if(s instanceof _tree._node){
                        ((_tree._node)s).node().remove();
                    } else{
    ...
    interface $java<_J extends _java._domain, $P extends $pattern> extends $pattern<_J, $P> {
        ...
        default List<_codeUnit> replaceIn(_project _codeProvider, $P $protoReplace ){
        ...
        default <_J extends _java._domain> _J replaceIn(_J _j , $P $protoReplace ){
            Walk.in(_j, _modelType(), e-> {
                ...
                if( sel != null ){
                    ...
                    _tree._node rep = (_tree._node)tj.draft( sel.tokens() );
                    _n.node().replace( rep.node() );
                }
            });
            return _j;
        }
        ...
        default <N extends Node> N replaceIn(N astNode, $P $protoReplace ){
            Walk.in( astNode, _modelType(), t->true, e-> {
                ...
                if( sel != null ){
                    ...
                    ((_tree._node)found._node()).node().replace( ((_tree._node)replacement).node() );
                }
    ...
    class $tokens implements Map<String, Object> {
        ...
        public boolean is(String $name, Object expectedValue) {
            ...
            if (expectedValue instanceof Expression) {
                return Expr.equal((Expression) expectedValue, get($name));
            } else if (expectedValue instanceof String) {
                try {
                    return Expr.equal(Expr.of((String) expectedValue), o);
                } catch (Exception e) {
