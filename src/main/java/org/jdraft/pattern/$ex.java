...
public class $ex<T extends Expression>
     ...
     */
    @Override
    public Select<T> selectFirstIn( _draft _j){
     ...
     * @return  the first Expression that matches (or null if none found)
     */
    public Select<T> selectFirstIn(_draft _j, Predicate<Select<T>> selectConstraint){
        if( _j instanceof _code ){
    ...
    @Override
    public List<T> listIn(_draft _j){
        if( _j instanceof _code ){
            if( ((_code) _j).isTopLevel()){
    ...
    @Override
    public List<Select<T>> listSelectedIn(_draft _j){
        List<Select<T>>sts = new ArrayList<>();
     ...
     * @return 
     */
    public <_J extends _draft> _J replaceIn(_J _j, Node astExprReplace ){
        _walk.in(_j, this.expressionClass, e-> {
            Select sel = select( e );
            if( sel != null ){
     ...
     * @return 
     */
    public <_J extends _draft> _J replaceIn(_J _j, String protoReplaceExpr ){
     ...
     * @return 
     */
    public <_J extends _draft> _J replaceIn(_J _j, $ex $replaceProto){
        _walk.in(_j, this.expressionClass, e-> {
            Select sel = select( e );
            if( sel != null ){
     ...
     * @return 
     */
    public <_J extends _draft> _J forSelectedIn(_J _j, Consumer<Select<T>> selectConsumer ){
        _walk.in(_j, this.expressionClass, e-> {
            Select sel = select( e );
            if( sel != null ){
     ...
     * @return 
     */
    public <_J extends _draft> _J forSelectedIn(_J _j, Predicate<Select<T>> selectConstraint, Consumer<Select<T>> selectConsumer ){
        _walk.in(_j, this.expressionClass, e-> {
            Select sel = select( e );
    ...
    */
     ...
     */
    public static class Select<T extends Expression> implements $pattern.selected,
            selectAst<T> {
