...
public interface _diffNode<_PN extends _java._domain> {
    ...
    default boolean on( Class<? extends _java._compound> clazz){
        return path().has(clazz);
    }
    
    default boolean on(Class<? extends _java._compound> clazz, String id){
     ...
     * @return 
     */
    default <_N extends _java._compound> boolean at(Class<_N> component, String id) {
        return path().isLeaf(component) && path().isLeafId(id);
    }

    default <_N extends _java._compound> boolean at(Class<_N> component) {
