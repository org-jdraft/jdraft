...
public class _methodsDiff
        implements _differ<List<_method>, _java._compound> {
    ...
    @Override
    public <_PN extends _java._compound> _diff diff(_nodePath path, _build dt, _PN _leftParent, _PN _rightParent, List<_method> left, List<_method> right) {
        Set<_method> ls = new HashSet<>();
        ls.addAll(left);
        Set<_method> rs = new HashSet<>();
        rs.addAll(right);
