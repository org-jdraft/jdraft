...
public class _annosDiff
    ...
    public <_PN extends _java._multiPart> _diff diff(_nodePath path, _build ds, _PN _leftParent, _PN _rightParent, _annos left, _annos right) {
        ...
        for (int i = 0; i < laes.size(); i++) {
            ...
            if (!raes.stream().filter(a -> Expressions.equivalent(e, (AnnotationExpr) a)).findFirst().isPresent()) {
        ...
        for (int i = 0; i < raes.size(); i++) {
            ...
            if (!laes.stream().filter(a -> Expressions.equivalent(e, (AnnotationExpr) a)).findFirst().isPresent()) {
