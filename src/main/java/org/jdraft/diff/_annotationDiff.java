...
public class _annotationDiff implements _differ<_annotation, _java._compoundNode> {

    public static final _annotationDiff INSTANCE = new _annotationDiff();

    @Override
    public <_PN extends _java._compoundNode> _diff diff(_nodePath path, _build ds, _PN _leftParent, _PN _rightParent, _annotation left, _annotation right) {
    ...
    public static class _annotationElementsDiff implements
            _differ<List<_annotation._entry>, _java._compoundNode> {
        ...
        @Override
        public <_PN extends _java._compoundNode> _diff diff(_nodePath path, _build ds, _PN _leftParent, _PN _rightParent, List<_annotation._entry> left, List<_annotation._entry> right) {
            Set<_annotation._entry> ls = new HashSet<>();
            Set<_annotation._entry> rs = new HashSet<>();
            ...
            ls.forEach(f -> {
                ...
                if (cc != null) {
                    rs.remove(cc);
                    ANNOTATION_ELEMENT_DIFF.diff(path.in(_java.Component.ELEMENT, f.getName()), ds, _leftParent, _rightParent, f, cc);
                } else {
                    ds.addDiff(new _leftOnly_element(path.in(_java.Component.ELEMENT, f.getName()), (_annotation) _leftParent, (_annotation) _rightParent, f));
                }
            });

            rs.forEach(f -> {
                ds.addDiff(new _rightOnly_element(path.in(_java.Component.ELEMENT, f.getName()), (_annotation) _leftParent, (_annotation) _rightParent, f));
            });
            return ds;
        }
    ...
    public static class _annotationElementDiff implements
            _differ<_annotation._entry, _java._compoundNode> {

        @Override
        public <_PN extends _java._compoundNode> _diff diff(_nodePath path, _build ds, _PN _leftParent, _PN _rightParent, _annotation._entry left, _annotation._entry right) {
            _javadocDiff.INSTANCE.diff(path, ds, left, right, left.getJavadoc(), right.getJavadoc());
            _annosDiff.INSTANCE.diff(path, ds, left, right, left.getAnnos(), right.getAnnos());
    ...
    public static class DefaultValueDiff implements _differ<Expression, _annotation._entry> {
        ...
        @Override
        public <_PN extends _java._compoundNode> _diff diff(_nodePath path, _build ds, _PN _leftParent, _PN _rightParent, Expression left, Expression right) {
            if (!Objects.equals(left, right)) {
                ds.addDiff(new _changeDefault(path.in(_java.Component.DEFAULT), (_annotation._entry) _leftParent, (_annotation._entry) _rightParent));
            }
            return (_diff) ds;
        }
    }
