...
public final class _nodePath {
        ...
        for (int i = 0; i < pathAsTokens.length; i += 2) {
            ...
            if ((pathAsTokens.length > i + 1) && pathAsTokens[i + 1] instanceof String) {
                if( pathAsTokens[i] instanceof Feature){
                    ...
                    _p = _p.in(Feature.of( (Class<_tree._node>)pathAsTokens[i]), (String) pathAsTokens[i + 1]);
                }
            } else {
                if( pathAsTokens[i] instanceof Feature){
                    _p = _p.in((Feature) pathAsTokens[i]);
                } else{
                    _p = _p.in( Feature.of( (Class<_tree._node>)pathAsTokens[i]));
                }
            }
        }
        return _p;
     ...
     * @return
     */
    public <_N extends _tree._node> boolean is(int index, Class<_N> clazz, String id) {
        if (index <= this.size() && index >= 0) {
     ...
     * @return
     */
    public <_N extends _tree._node> boolean isLeaf(Class<_N> clazz, String id) {
     ...
     * @return
     */
    public <_N extends _tree._node> boolean isLeaf(Class<_N> clazz) {
     ...
     * @return
     */
    public <_N extends _tree._node> boolean has(Class<_N> clazz) {
     ...
     * @return
     */
    public <_N extends _tree._node> boolean has(Class<_N> clazz, String id) {
        for (int i = 0; i < size(); i++) {
            if (this.featurePath.get(i).implementationClass.equals(clazz)
                    && this.idPath.get(i).equals(id)) {
                return true;
